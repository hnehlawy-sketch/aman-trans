...
Dependencies: python-docx reportlab wkhtmltopdf(system)
...
def pip_install_fallback(*pkgs):
    sys.stderr.write(f"[aman] WARNING: Installing missing packages: {pkgs}\n")
    cmd = [sys.executable, '-m', 'pip', 'install', '--quiet',
           '--disable-pip-version-check', *pkgs]
    if sys.platform != 'win32': cmd.append('--break-system-packages')
...
def find_arabic_ttf():
    ...
    for d in ['/usr/share/fonts', r'C:\Windows\Fonts', '/Library/Fonts']:
        ...
        for root, _, files in os.walk(d):
            for f in files:
                if f.lower().endswith('.ttf'): return os.path.join(root, f)
    return None

def pdf_via_wk(text, out, wk):
    direction = detect_dir(text)
    is_rtl = direction == 'rtl'
    font_stack = ('"Arabic Typesetting","Traditional Arabic","Simplified Arabic",Arial,Tahoma,sans-serif'
    ...
    for line in text.split('\n'):
        ...
        elif re.match(r'^\[(?:Page|صفحة)\s*\d+', s, re.I):
...
body{{font-family:{font_stack};direction:{direction};text-align:{'right' if is_rtl else 'left'};
     font-size:13pt;line-height:2;color:#1a1a1a;unicode-bidi:embed}}
p{{margin-bottom:8px;word-break:break-word}}
h1{{font-size:20pt;font-weight:bold;margin:18px 0 10px;border-bottom:1px solid #ccc}}
h2{{font-size:16pt;font-weight:bold;margin:14px 0 8px}}
h3{{font-size:14pt;font-weight:bold;margin:12px 0 6px}}
.pgmark{{color:#888;font-size:10pt;border-top:1px dashed #ddd;margin:14px 0;padding-top:6px}}
</style></head>
...
def pdf_via_reportlab(text, out):
    ...
    base  = ParagraphStyle('B', fontName=fname, fontSize=12, leading=22,
                            alignment=align, wordWrap='RTL' if is_rtl else 'LTR', spaceAfter=6)
    h1    = ParagraphStyle('H1', parent=base, fontSize=18, spaceBefore=12)
    h2    = ParagraphStyle('H2', parent=base, fontSize=15, spaceBefore=10)
    h3    = ParagraphStyle('H3', parent=base, fontSize=13, spaceBefore=8)
    ...
    doc.build(story)
    return {'success': True, 'method': 'reportlab_fallback', 'dir': direction,
            'note': 'Install wkhtmltopdf from https://wkhtmltopdf.org for better Arabic'}
    ...
    for (_, para), new_text in zip(text_paras, parts):
        ...
        if is_rtl:
            ...
            for tag in ['w:bidi', 'w:jc']:
                el = pPr.find(qn(tag))
                if el is None: el = OxmlElement(tag); pPr.insert(0, el)
        ...
        if runs:
            ...
            if is_rtl:
                ...
                for tag in ['w:rtl', 'w:cs']:
                    if rPr.find(qn(tag)) is None: rPr.append(OxmlElement(tag))
                rFonts = rPr.find(qn('w:rFonts'))
                if rFonts is None: rFonts = OxmlElement('w:rFonts'); rPr.insert(0, rFonts)
                for a in ['w:ascii', 'w:hAnsi', 'w:cs']: rFonts.set(qn(a), 'Arial')
...
def main():
    ...
    orig, trans, out = sys.argv[1], sys.argv[2], sys.argv[3]
    ext = os.path.splitext(orig)[1].lower()

    try:
        with open(trans, 'r', encoding='utf-8') as f: text = f.read()

        if ext in ('.docx', '.doc'): result = rebuild_docx(orig, text, out)
        elif ext == '.pdf':
            wk = find_wk()
            result = pdf_via_wk(text, out, wk) if wk else pdf_via_reportlab(text, out)
        else:
